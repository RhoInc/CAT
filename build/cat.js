(function (global, factory) {
  ...
  function addControlsToggle() {
      ...
      this.controls.minimize = this.controls.submitWrap.append('div').classed('cat-button cat-button--minimize hidden', true).attr('title', 'Hide controls').text('<<').on('click', function () {
          ...
          _this.controls.maximize = _this.wrap.insert('div', ':first-child').classed('cat-button cat-button--maximize', true).text('>>').attr('title', 'Show controls').on('click', function () {
  ...
  function getVersions(select) {
      var repo = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : 'https://www.github.com/RhoInc/Webcharts';

      console.log(this);
      console.log(select);
      console.log(repo);
      var api = repo.replace('www.github.com', 'api.github.com/repos');
      var branches = fetch(api + '/branches').then(function (response) {
          return response.json();
      });
      var releases = fetch(api + '/releases').then(function (response) {
          return response.json();
      });
  ...
  function initRendererSelect(cat) {
      ...
      getVersions(cat.controls.versionSelect, cat.current.url);
