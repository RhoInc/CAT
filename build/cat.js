...
function layout(cat) {
  ...
  cat.dataWrap = cat.wrap.append("div").attr("class", "cat-data section").classed("hidden", true);

  /* Layout CAT Controls Divs */
  cat.controls.wrap.append("h2").text("Charting Application Tester 😼");

  cat.controls.submitWrap = cat.controls.wrap.append("div").attr("class", "control-section submit-section");

  cat.controls.rendererWrap = cat.controls.wrap.append("div").attr("class", "control-section renderer-section");

  cat.controls.dataWrap = cat.controls.wrap.append("div").attr("class", "control-section data-section");

  cat.controls.settingsWrap = cat.controls.wrap.append("div").attr("class", "control-section settings-section");

  cat.controls.environmentWrap = cat.controls.wrap.append("div").attr("class", "control-section environment-section");
}
...
function loadRenderer(cat) {
  ...
  if (rendererObj.css) {
    var cssPath = cat.config.rootURL + "/" + rendererObj.name + "/" + version + "/" + rendererObj.css;
  ...
  if (!js_loaded) {
    ...
    loader.require(rendererPath, {
      ...
      failure: function failure() {
        cat.status.loadStatus(cat.statusDiv, false, rendererPath, rendererObj.name, version);
      }
    });
  } else {
    cat.status.loadStatus(cat.statusDiv, true, rendererPath, rendererObj.name, version);
...
function loadLibrary(cat) {
  ...
  // --- load css --- //
  var cssPath = cat.config.rootURL + "/" + "webcharts" + "/" + version + "/" + "css" + "/" + "webcharts.css";
  ...
  // --- load js --- //
  var rendererPath = cat.config.rootURL + "/" + library + "/" + version + "/build/" + "webcharts" + ".js";
...
function initSubmit(cat) {
  cat.controls.submitButton = cat.controls.submitWrap.append("button").attr("class", "submit").text("Render Chart").on("click", function () {
    cat.dataWrap.classed("hidden", true);
    cat.chartWrap.classed("hidden", false);
...
function addEnterEventListener(cat) {
    //Add Enter event listener to all controls.
    cat.controls.wrap.selectAll('select,input').each(function () {
...
function set$1(cat) {
  ...
  d3.json(schemaPath, function (error, schemaObj) {
    ...
    cat.controls.settingsInput.node().value = JSON5.stringify(cat.current.config);
...
function sync(cat, printStatus) {
  ...
  if (cat.current.settingsView == "text") {
    ...
    if (IsJsonString(jsonText)) {
      ...
      cat.controls.settingsInput.node().value = JSON.stringify(JSON5.parse(jsonText));
